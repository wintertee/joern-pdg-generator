...
def main():
    ...
    nodes_to_remove = [node for node, data in graph.nodes(data=True) if data.get("label") not in node_filter.node_labels]
    ...
    edges_to_remove = [
        (u, v, k) for u, v, k, data in graph.edges(keys=True, data=True) if data.get("label") not in edge_filter.edge_labels
    ]
